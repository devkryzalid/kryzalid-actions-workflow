...
| `build-laravel.yml` | Laravel générique (API seule ou avec front Blade/Vite) | SSH → `git reset --hard origin/<branch>` → `composer install --no-dev --optimize-autoloader` (via `php_bin`) → clear caches (délègue à `cc.sh` du projet) → migrate si `run_migrations` → build npm/Vite si `build_assets` → re-cache config/view/route selon `cache_config`/`cache_views`/`cache_routes` → `artisan queue:restart` si `restart_queue_worker`. Toutes les étapes optionnelles sont harmonisées en `default: true` sauf `restart_queue_worker` (`default: false`, tous les projets n'ont pas de worker) — désactiver au cas par cas via l'input correspondant plutôt que de bifurquer le workflow. Inputs : `build_assets`, `restart_queue_worker`, `php_bin` (default `php`), `run_migrations`, `cache_config`, `cache_views`, `cache_routes`. `cache_routes` à désactiver si les routes sont enregistrées dynamiquement par requête (ex: `mcamara/laravel-localization` avec `LaravelLocalization::setLocale()` sans argument dans `RouteServiceProvider::boot()` — le cache fige tout sur ce qu'un contexte CLI résout, différent d'une vraie requête HTTP). `cache_config` à désactiver si `config:cache` produit des valeurs vides pour certaines clés `env()` en contexte SSH non-interactif (observé sur teedy avec `DB_USERNAME`/`DB_PASSWORD` alors que `env()`/`config()` résolvent correctement hors `config:cache` — cause racine non élucidée, `config:cache` re-bootstrape l'app en interne d'une façon qui semble se comporter différemment ; désactiver est le contournement sûr). |
