...
| `build-laravel.yml` | Laravel générique (API seule ou avec front Blade/Vite) | SSH → `git reset --hard origin/<branch>` → `composer install --no-dev --optimize-autoloader` (via `php_bin`) → clear caches (délègue à `cc.sh` du projet) → `artisan migrate --force` → build npm/Vite si `build_assets` (default `true`) → re-cache config/view (+ route si `cache_routes`, default `true`) → `artisan queue:restart` si `restart_queue_worker` (default `false`). Inputs paramétrables : `build_assets`, `restart_queue_worker`, `php_bin` (default `php`), `cache_routes` (default `true` — à désactiver si les routes sont enregistrées dynamiquement par requête, ex: `mcamara/laravel-localization` avec `LaravelLocalization::setLocale()` sans argument dans `RouteServiceProvider::boot()` — le cache fige tout sur ce qu'un contexte CLI résout, différent d'une vraie requête HTTP ; incompatible avec ce pattern par design, pas juste un bug à corriger côté projet). |
