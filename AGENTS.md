...
| `build-laravel.yml` | Laravel générique (API seule ou avec front Blade/Vite) | SSH → `git reset --hard origin/<branch>` → `composer install --no-dev --optimize-autoloader` (via `php_bin`) → clear caches → `artisan migrate --force` → build npm/Vite si `build_assets` (default `true`) → re-cache config/route/view → `artisan queue:restart` si `restart_queue_worker` (default `false`). Inputs paramétrables : `build_assets`, `restart_queue_worker`, `php_bin` (default `php` — à surcharger sur serveur multi-PHP, ex: `php84`). |
