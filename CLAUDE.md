...
|---|---|---|
| `build-wordpress.yml` | WP avec thème Kryzaplate (Webpack/Vite) | SSH → `git reset --hard origin/<branch>` → **si le commit entrant a un `composer.json` racine** : contrôles **avant le reset** (binaires, `composer.lock` présent, aucun `auth.json` à la racine web, identifiants présents hôte par hôte dans le `auth.json` global Composer ou `COMPOSER_AUTH`) puis, après le reset, `composer install --no-dev` (via l'input `php_bin`, default `php`) → auto-détection du dossier contenant `package.json` (priorité `wp-content/themes/*`) → `npm ci` + `npm run dev` (staging) ou `npm run build` (prod). L'étape Composer n'est pas optionnelle : le reset supprime les dossiers de plugins qui étaient suivis par git, le site tournerait sans eux. D'où les contrôles avant le reset : un échec laisse le site intact. Composer crée un `auth.json` global vide au premier lancement, d'où le contrôle du contenu et pas de la présence du fichier. Les projets sans `composer.json` racine ne changent pas. Le `vendor/` du thème n'est **pas** installé par ce workflow. |
...
| `build-laravel.yml` | Laravel générique (API seule ou avec front Blade/Vite) | SSH → `git reset --hard origin/<branch>` (toujours, seule étape non désactivable — sinon la CI ne sert à rien) → `composer install --no-dev --optimize-autoloader` si `run_composer_install` (via `php_bin`) → clear caches (`cc.sh` du projet) si `clear_caches` → migrate si `run_migrations` → build npm/Vite si `build_assets` → re-cache config/view/route selon `cache_config`/`cache_views`/`cache_routes` → `artisan queue:restart` si `restart_queue_worker`. Toutes les étapes optionnelles sont harmonisées en `default: true` sauf `restart_queue_worker` (`default: false`, tous les projets n'ont pas de worker) — désactiver au cas par cas via l'input correspondant plutôt que de bifurquer le workflow. Inputs : `build_assets`, `restart_queue_worker`, `php_bin` (default `php`), `run_migrations`, `run_composer_install`, `clear_caches`, `cache_config`, `cache_views`, `cache_routes`. `cache_routes` à désactiver si les routes sont enregistrées dynamiquement par requête (ex: `mcamara/laravel-localization` avec `LaravelLocalization::setLocale()` sans argument dans `RouteServiceProvider::boot()` — le cache fige tout sur ce qu'un contexte CLI résout, différent d'une vraie requête HTTP). `cache_config` à désactiver si `config:cache` produit des valeurs vides pour certaines clés `env()` en contexte SSH non-interactif (observé sur teedy avec `DB_USERNAME`/`DB_PASSWORD` alors que `env()`/`config()` résolvent correctement hors `config:cache` — cause racine non élucidée, `config:cache` re-bootstrape l'app en interne d'une façon qui semble se comporter différemment ; désactiver est le contournement sûr). Notification Slack : liste chacune des 9 étapes ci-dessus avec son statut (✅ exécutée / ⏭️ ignorée) à chaque déploiement. |
